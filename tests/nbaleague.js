...
contract NBALeague {
    ...
    function createTournament (string name, uint fee) constant returns (string){
        Each e = allCompetitions[numTournaments];
        e.tournament = new Tournament(name);
        e.num = numTournaments;
        e.tournament.setAll(fee);
        numTournaments++;
        return strConcat(name, " tournament created successfully.");
    }
    
    function createCategories (uint numTournament, 
    ...
    function addPlayer
    (uint numTournament, address user, string playerName, string playerPos) returns (string) {
        if (allCompetitions[numTournament].tournament.addPlayerToRoster(user, playerName, playerPos)) {
    ...
    (uint numTournament, address user, string playerName, string playerPos) returns (string) {
        if (allCompetitions[numTournament].tournament.deletePlayer(user, playerName, playerPos)) return "Deleted successfully";
...
contract Tournament{
    ...
    function addPlayerToRoster(address addr, string playerName, string playerPos) locked returns (bool) {
       ...
       if (stringsEqual(playerPos, "PG")) {
           if (!u.roster[0].initialized) {
               ...
               if (stringsEqual(playerName, u.roster[0].name)) return false;
       ...
       if (stringsEqual(playerPos, "SG")) {
           if (!u.roster[2].initialized) {
               ...
               if (stringsEqual(playerName, u.roster[2].name)) return false;
       ...
       if (stringsEqual(playerPos, "C")) {
           if (!u.roster[4].initialized) {
               ...
               if (stringsEqual(playerName, u.roster[4].name)) return false;
       ...
       if (stringsEqual(playerPos, "SF")) {
           if (!u.roster[6].initialized) {
               ...
               if (stringsEqual(playerName, u.roster[6].name)) return false;
       ...
       if (stringsEqual(playerPos, "PF")) {
           if (!u.roster[8].initialized) {
               ...
               if (stringsEqual(playerName, u.roster[8].name)) return false;
    ...
    function deletePlayer(address addr, string playerName, string playerPos) locked returns (bool) {
        User u = listUsers[addr];
        if (stringsEqual(playerPos, "PG")) {
      ...
      else if (stringsEqual(playerPos, "SG")) {
       ...
       else if (stringsEqual(playerPos, "C")) {
       ...
       else if (stringsEqual(playerPos, "SF")) {
       ...
       else if (stringsEqual(playerPos, "PF")) {
